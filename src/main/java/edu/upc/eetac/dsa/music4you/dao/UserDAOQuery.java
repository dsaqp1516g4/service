...
<<<<<<< HEAD
    public final static String CREATE_MSG = "insert into mensajeria (id,userid,destinatario,text) values (unhex(?),unhex(?),?,?)";
    public final static String CHECK_MSGS ="select hex(userid) as userid, users.fullname as fromusername, count(*) as count,creation_timestamp from mensajeria left join users ON users.id=mensajeria.userid where destinatario=unhex(?) group by userid order by creation_timestamp desc";
...
=======
    public final static String CREATE_MSG = "insert into mensajeria (id, userid ,destinatario ,body) values (unhex(?),unhex(?),unhex(?),?)";
    public final static String CHECK_MSGS ="select hex(destinatario) as userid, users.fullname as loginid, count(*) as count,creation_timestamp from mensajeria left join users ON users.id=mensajeria.userid  where destinatario=unhex(?) group by userid order by creation_timestamp desc";
    public final static String READ_MSGS ="select creation_timestamp, hex(userid) as userid, users.fullname as loginid,hex(destinatario) as destinatario,body from mensajeria left join users ON users.id=mensajeria.userid where (destinatario=unhex(?) and userid=unhex(?)) or (destinatario=unhex(?) and userid=unhex(?)) order by creation_timestamp desc;";
