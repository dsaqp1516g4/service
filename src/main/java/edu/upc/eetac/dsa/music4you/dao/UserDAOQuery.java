...
public interface UserDAOQuery {
    ...
    public final static String CHECK_MSGS ="select hex(destinatario) as userid, users.fullname as loginid, count(*) as count,creation_timestamp from messages left join users ON users.id=mensajeria.userid  where destinatario=unhex(?) group by userid order by creation_timestamp desc";
