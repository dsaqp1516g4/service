...
=======
    public final static String GET_PLAY_BY_ID = "select hex(p.id) as id, hex(p.userid) as userid, p.title, p.artist, p.genre, p.creation_timestamp, p.last_modified, u.id from playlist p, users u where p.id=unhex(?) and u.id=p.userid";
    public final static String GET_PLAYLIST_BY_USER = "select hex(p.id) as id, hex(p.userid) as userid, p.artist, p.creation_timestamp, p.last_modified, u.id from playlist p, users u where creation_timestamp < ? and u.id=p.userid order by creation_timestamp desc limit 5";
    public final static String GET_PLAYLIST_AFTER_BY_USER = "select hex(p.id) as id, hex(p.userid) as userid, p.artist, p.creation_timestamp, p.last_modified, u.id from playlist p, users u where creation_timestamp > ? and u.id=p.userid order by creation_timestamp desc limit 5";
    public final static String GET_PLAYLIST_BY_ARTIST = "select hex(id) as id, hex(userid) as userid, title, artist, genre, creation_timestamp, last_modified from playlist where artist=? and creation_timestamp < ? order by creation_timestamp desc limit 5";
    public final static String GET_PLAYLIST_BY_GENRE = "select hex(id) as id, hex(userid) as userid, title, artist, genre, creation_timestamp, last_modified from playlist where genre=? and creation_timestamp < ? order by creation_timestamp desc limit 5";
    public final static String GET_PLAYLIST = "select hex(id) as id, hex(userid) as userid, title, artist, creation_timestamp, last_modified from playlist where creation_timestamp < ? order by creation_timestamp desc limit 5";
