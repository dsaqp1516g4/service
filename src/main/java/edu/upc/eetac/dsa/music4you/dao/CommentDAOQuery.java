...
public class CommentDAOQuery {
    ...
    public final static String INSERT_COMMENT_QUERY = "insert into comments (id, userid, anuncioid, eventid, content) values (unhex(?), unhex(?), unhex(?), unhex(?), ?)";
    ...
    public final static String GET_COMMENTS_EVENT_QUERY = "select hex(c.id) as id, hex(c.userid) as userid, hex(c.eventid) as eventid, c.content, c.creation_timestamp, c.last_modified, u.fullname from comments c, users u where u.id=c.userid and c.creation_timestamp < ifnull(?, now()) and c.eventid = unhex(?) order by creation_timestamp";
    public final static String GET_COMMENTS_EVENT_QUERY_FROM_LAST="select hex(c.id) as id, hex(c.userid) as userid, hex(c.eventid) as eventid, c.content, c.creation_timestamp, c.last_modified, u.fullname from comments c, users u where u.id=c.userid and c.creation_timestamp < ifnull(?, now()) and c.eventid = unhex(?) order by creation_timestamp";
}
